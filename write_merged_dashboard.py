...
class DashboardScreen extends ConsumerWidget {
      ...
      data: (applications) {
        return DefaultTabController(
          ...
          child: Scaffold(
            appBar: const TabBar(
              tabs: [
                Tab(text: 'Aktuelle Woche'),
                Tab(text: 'Gesamtübersicht'),
              ],
            ),
            body: TabBarView(
              children: [
              ...
              if (constraints.maxWidth < 600) {
                return Column(
                  children: [
                    Row(
                      children: [
                        _buildStatCard(context, thisWeekApps.length.toString(), 'NEUE BEWERBUNGEN'),
                        const SizedBox(width: 8),
                        _buildStatCard(context, stats.open.toString(), 'AKTIVE BEWERBUNGEN'),
                      ]
                    ),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        _buildStatCard(context, thisWeekRejections.toString(), 'ABSAGEN'),
                        const SizedBox(width: 8),
                        const Spacer(),
                      ],
                    ),
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _StatCard(title: 'BEWERBUNGEN', value: '$total'),
                _StatCard(title: 'NOCH OFFEN', value: '$open'),
                _StatCard(title: 'ABSAGEN', value: '$rejected'),
                _StatCard(title: 'ANTWORTQUOTE', value: '${responseRate.toStringAsFixed(1)}%'),
                _StatCard(title: 'ABSAGEQUOTE', value: '${rejectionRate.toStringAsFixed(1)}%'),
                _StatCard(title: 'INTERVIEWS', value: '$interviews'),
                _StatCard(title: 'Ø PENDELZEIT AUTO', value: '${avgCommute.toStringAsFixed(0)} Min'),
              ],
            ),
          ),
          const SizedBox(height: 32),
          Text('BEWERBUNGEN PRO MONAT', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 16),
          Card(
            child: Padding(
              padding: const EdgeInsets.all(16.0),
                          ...
                          getTitlesWidget: (double value, TitleMeta meta) {
                            if (value.toInt() >= 0 && value.toInt() < monthlyCounts.keys.length) {
                              return Padding(
                                ...
                                child: Text(monthlyCounts.keys.elementAt(value.toInt()), style: const TextStyle(fontSize: 10)),
                              );
                            }
                            return const Text('');
                          },
          ...
          const SizedBox(height: 32),
          Text('TOP ABSAGEGRÜNDE', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 16),
          Card(
            child: sortedReasons.isEmpty
                ? const Padding(
                    padding: EdgeInsets.all(16.0),
                    child: Text('Bisher keine Absagegründe erfasst.'),
                  )
                : ListView.builder(
                    shrinkWrap: true,
  ...
  Widget _buildMotivationalHeader(BuildContext context, int count) {
    String message;
    if (count == 0) {
      message = 'Jede Reise beginnt mit dem ersten Schritt! ??';
    } else if (count <= 2) {
      message = 'Guter Start! Weiter so! ??';
    } else if (count <= 4) {
      message = 'Starke Leistung diese Woche! ??';
    } else {
      message = 'FANTASTISCHE ARBEIT DIESE WOCHE! ??';
    }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Wochenziel: $current von $goal Bewerbungen',
...
with open('lib/presentation/screens/dashboard/dashboard_screen.dart', 'w', encoding='utf-8') as f:
    f.write(content)
