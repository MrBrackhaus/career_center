...
final streakProvider = Provider<AsyncValue<StreakData>>((ref) {
  ...
  if (applicationsAsync.hasError) {
    return AsyncValue.error(applicationsAsync.error!, applicationsAsync.stackTrace!);
  }
  ...
  return AsyncValue.data(StreakData(
    streakCount: streak,
    weeklyGoal: goal,
...
String _getIsoWeekKey(DateTime date) {
  ...
  final int dayOfYear = int.parse(date.difference(DateTime(date.year, 1, 1)).inDays.toString());
  ...
  if (week < 1) {
    ...
    if (dec31.weekday == DateTime.thursday || (dec31.weekday == DateTime.friday && _isLeapYear(date.year))) {
      week = 53;
    } else {
      year++;
      week = 1;
